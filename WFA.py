...
def calculate_average_input_distance(X):
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1] - 1):
            L_2_distance += np.sqrt(np.sum(abs(X[i, j+1] - X[i, j]) ** 2))
    average_L_2_distance = L_2_distance / (X.shape[0] * (X.shape[1] - 1))
    return average_L_2_distance
...
def build_WFA(model, X, Y, K=3, T=30, F=0.1, details=False):
    ...
    for item in range(1, abst_states_number + 1):  # for every abstract state
        ...
        final_vector[item, :] = item_classes / item_sum  # count the probability of every class
...
if __name__ == '__main__':
    from models.models_structure.AtrialFibrillation import RNN
    model = torch.load('models/AtrialFibrillation.pkl')
    ...
    abst_alphabet, initial_vec, trans_matrices, final_vec = build_WFA(model, X, Y, K=2, T=20, F=1, details=True)

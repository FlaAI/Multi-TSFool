...
def TSFool(model, X, Y, K=2, T=30, F=0.1, eps=0.1, N=20, P=0.9, C=1, target=-1, details=False):
        ...
        if model_pred_y[i] != Y[i] and rep_model_pred_y[i] == Y[i]:
            ...
            for candidate_sample_index in candidate_samples_index:
                ...
                while True:
                    ...
                    end_flag = True
                    for j in range(X.shape[1]):
                        current_L_2_distance = np.sqrt(np.sum(sampling_bias_x[j] ** 2))
                        if current_L_2_distance > perturbation_amount / 10:
                            end_flag = False
                            break
                    if end_flag:
    ...
    for i in range(len(minimum_positive_sample_X)):
        ...
        current_adv_X = []
        for j in range(N):
            current_mask = np.random.choice([0, 1], size=(X.shape[1] * X.shape[2]), p=[(1 - P), P]).reshape(-1, X.shape[2])
...
if __name__ == '__main__':
    ...
    adv_X, adv_Y, target_X = TSFool(model, X, Y, K=2, T=20, F=1, eps=0.01, N=20, P=0.9, C=1, target=-1, details=False)
